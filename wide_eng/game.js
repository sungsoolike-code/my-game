...
const DIFFICULTY = {
  easy: {
    label: 'Easy',
    color: 0x44cc44,
    desc: 'More HP, slower enemies — for beginners',
  ...
  normal: {
    label: 'Normal',
    color: 0xddaa22,
    desc: 'Balanced default difficulty',
  ...
  hard: {
    label: 'Hard',
    color: 0xdd4444,
    desc: 'Fast & many enemies — for veterans',
...
function createSettingsPanel(scene, cx, cy, els, onBack) {
  ...
  // Next track hint
  const nextTrackHint = scene.add.text(cx, cy + 52, 'Press 1: Next Track', {
  ...
  const backLabel = scene.add.text(cx, cy + 100, 'Back', {
...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(cx, VIEW_H - 40, 'WASD: Move  R: Bomb  F: Armor  1: Next Track  SPACE: Pause', {
...
class GameScene extends Phaser.Scene {
  ...
  updateUI() {
    ...
    this.timerText.setText(`${mm}:${ss}  (Next in ${this.waveTimer}s)`);
  ...
  showPauseMenu() {
    ...
    const hint = this.add.text(VIEW_W / 2, VIEW_H / 2 - 75, 'Press SPACE to continue', {
      fontSize: '14px', fill: '#888888', fontFamily: 'monospace',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(301);
    els.push(hint);
    ...
    const resumeLabel = this.add.text(VIEW_W / 2, VIEW_H / 2 - 20, 'Resume', {
    ...
    const settingsBg = this.add.rectangle(VIEW_W / 2, VIEW_H / 2 + 45, 260, 50, 0x335555)
      .setScrollFactor(0).setDepth(301).setInteractive({ useHandCursor: true });
    const settingsLabel = this.add.text(VIEW_W / 2, VIEW_H / 2 + 45, 'Settings', {
      fontSize: '22px', fill: '#ffffff', fontFamily: 'monospace',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(302);
    ...
    const menuBg = this.add.rectangle(VIEW_W / 2, VIEW_H / 2 + 110, 260, 50, 0x444466)
      .setScrollFactor(0).setDepth(301).setInteractive({ useHandCursor: true });
    const menuLabel = this.add.text(VIEW_W / 2, VIEW_H / 2 + 110, 'Main Menu', {
      fontSize: '22px', fill: '#ffffff', fontFamily: 'monospace',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(302);
...
class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    const restartBg = this.add.rectangle(cx, cy + 120, 240, 50, 0x446644)
      .setInteractive({ useHandCursor: true });
    this.add.text(cx, cy + 120, 'Restart', {
    ...
    const menuBg = this.add.rectangle(cx, cy + 180, 240, 50, 0x444466)
      .setInteractive({ useHandCursor: true });
    this.add.text(cx, cy + 180, 'Main Menu', {
      fontSize: '24px', fill: '#ffffff', fontFamily: 'monospace',
    }).setOrigin(0.5);
    menuBg.on('pointerover', () => menuBg.setFillStyle(0x6666aa));
    menuBg.on('pointerout', () => menuBg.setFillStyle(0x444466));
