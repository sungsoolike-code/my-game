...
// ============================================================
const MusicManager = {
  _scene: null, _tracks: [], _currentIndex: -1, _currentSound: null, _shuffled: [], _loaded: false,
  init(scene) {
    this._scene = scene;
  ...
  _playNext() {
    ...
    if (!scene.cache.audio.exists(key)) {
      ...
      scene.load.once('complete', () => this._startTrack(scene, key));
      scene.load.start();
    } else {
  ...
  _startTrack(scene, key) {
    if (this._currentSound) { this._currentSound.stop(); this._currentSound.destroy(); }
    this._currentSound = scene.sound.add(key, { volume: 0.3 });
    this._currentSound.once('complete', () => this._playNext());
    if (SETTINGS.musicOn) this._currentSound.play();
  },
  stop() { if (this._currentSound && this._currentSound.isPlaying) this._currentSound.stop(); },
  toggle(on) { SETTINGS.musicOn = on; if (on) this.play(); else this.stop(); },
  updateScene(scene) { this._scene = scene; },
};
...
function createSettingsPanel(scene, cx, cy, els, onBack) {
  ...
  const soundBtn = scene.add.rectangle(cx + 80, cy - 42, 100, 36, SETTINGS.soundOn ? 0x44aa44 : 0x664444)
    .setStrokeStyle(1, 0x888888).setScrollFactor(0).setDepth(401).setInteractive({ useHandCursor: true });
  ...
  create() {
    ...
    keys.forEach((key, i) => {
      ...
      const bg = this.add.rectangle(cx, y, 400, 72, diff.color, 0.25)
        .setStrokeStyle(2, diff.color).setInteractive({ useHandCursor: true });
    ...
    this.add.text(cx, VIEW_H - 40, '터치로 이동  |  공격은 자동  |  R버튼: 폭탄', {
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.spawnEvent = this.time.addEvent({
      delay: d.spawnDelay, callback: this.spawnEnemy, callbackScope: this, loop: true,
    });
  ...
  createTouchControls() {
    // 조이스틱 (왼쪽 하단)
    const joyX = 140, joyY = VIEW_H - 140;
    const joyRadius = 60;
    const baseRadius = 100;

    // 조이스틱 베이스
    this.joyBase = this.add.circle(joyX, joyY, baseRadius, 0xffffff, 0.08)
      .setScrollFactor(0).setDepth(500);
    this.joyStick = this.add.circle(joyX, joyY, joyRadius, 0xffffff, 0.25)
      .setScrollFactor(0).setDepth(501);
    this.joyBaseX = joyX;
    this.joyBaseY = joyY;
    this.joyMaxDist = baseRadius - joyRadius / 2;

    // R 폭탄 버튼 (오른쪽 하단)
    const btnX = VIEW_W - 100, btnY = VIEW_H - 100;
    this.bombBtn = this.add.circle(btnX, btnY, 40, 0xff4400, 0.3)
      .setStrokeStyle(3, 0xff6600, 0.6)
      .setScrollFactor(0).setDepth(500)
      .setInteractive();
    this.bombBtnLabel = this.add.text(btnX, btnY, 'R', {
      fontSize: '28px', fill: '#ffffff', fontFamily: 'monospace', fontStyle: 'bold',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(501);
    this.bombBtn.on('pointerdown', () => {
      if (!this.paused) this.tryFireBomb();
    });

    // 일시정지 버튼 (오른쪽 상단)
    const pauseX = VIEW_W - 50, pauseY = 50;
    this.pauseBtn = this.add.rectangle(pauseX, pauseY, 60, 40, 0x333333, 0.4)
    ...
    this.input.on('pointerdown', (pointer) => {
      if (this.paused) return;
      // 왼쪽 절반 터치 = 조이스틱
      if (pointer.x < VIEW_W / 2 && this.joystickPointerId === null) {
    ...
    this.input.on('pointermove', (pointer) => {
      ...
      if (pointer.id === this.joystickPointerId) {
        ...
        this.joyStick.x = this.joyBaseX + Math.cos(angle) * clampDist;
        this.joyStick.y = this.joyBaseY + Math.sin(angle) * clampDist;

        if (dist > 10) {
          this.touchVx = Math.cos(angle) * (clampDist / this.joyMaxDist);
          this.touchVy = Math.sin(angle) * (clampDist / this.joyMaxDist);
        } else {
          this.touchVx = 0;
          this.touchVy = 0;
        }
    ...
    this.input.on('pointerup', (pointer) => {
      if (pointer.id === this.joystickPointerId) {
        this.joystickPointerId = null;
        this.joyStick.x = this.joyBaseX;
        this.joyStick.y = this.joyBaseY;
        this.touchVx = 0;
        this.touchVy = 0;
      }
    });
  ...
  createUI() {
    ...
    this.waveText = this.add.text(VIEW_W - 80, 10, '', style).setOrigin(1, 0).setScrollFactor(0).setDepth(100);
    this.timerText = this.add.text(VIEW_W - 80, 30, '', style).setOrigin(1, 0).setScrollFactor(0).setDepth(100);
    this.killText = this.add.text(VIEW_W - 80, 50, '', style).setOrigin(1, 0).setScrollFactor(0).setDepth(100);
    this.buffText = this.add.text(20, 48, '', { fontSize: '13px', fill: '#ffdd00', fontFamily: 'monospace' }).setScrollFactor(0).setDepth(100);
    this.bombText = this.add.text(VIEW_W - 80, 70, '', { fontSize: '16px', fill: '#ff8844', fontFamily: 'monospace' }).setOrigin(1, 0).setScrollFactor(0).setDepth(100);
    this.healCountText = this.add.text(VIEW_W - 80, 90, '', { fontSize: '13px', fill: '#88aa88', fontFamily: 'monospace' }).setOrigin(1, 0).setScrollFactor(0).setDepth(100);

    const dc = this.diff.color;
    const dHex = '#' + dc.toString(16).padStart(6, '0');
    this.add.text(VIEW_W / 2, 10, `${this.diff.label}  [Mobile]`, {
  ...
  suiciderExplode(enemy) {
    ...
    const cells = [
      { gx: gridX, gy: gridY }, { gx: gridX + 1, gy: gridY },
      { gx: gridX, gy: gridY + 1 }, { gx: gridX + 1, gy: gridY + 1 },
    ];
    SFX.bombExplode();
    const fx = this.add.graphics(); fx.setDepth(50);
    cells.forEach(c => {
      fx.fillStyle(0xff2222, 0.4); fx.fillRect(c.gx * 64, c.gy * 64, 64, 64);
      fx.lineStyle(2, 0xff4444, 0.8); fx.strokeRect(c.gx * 64, c.gy * 64, 64, 64);
    });
    ...
    this.enemies.getChildren().forEach(e => {
      if (!e.active) return;
      const eGridX = Math.floor(e.x / 64), eGridY = Math.floor(e.y / 64);
      for (const c of cells) { if (eGridX === c.gx && eGridY === c.gy) { toKill.push(e); break; } }
    });
    toKill.forEach(e => { this.spawnDeathParticles(e.x, e.y, e.fillColor); this.addKill(); e.destroy(); });
  }
  ...
  onBulletHitEnemy(bullet, enemy) {
    ...
    if (pierce <= 0) bullet.destroy(); else bullet.setData('pierce', pierce);
    this.tweens.add({ targets: enemy, alpha: 0.3, yoyo: true, duration: 60 });

    if (hp <= 0) {
      if (enemy.getData('suicider')) { this.suiciderExplode(enemy); return; }
      SFX.enemyDeath();
      this.spawnDeathParticles(enemy.x, enemy.y, enemy.fillColor);
      this.addKill();
      const healOrb = this.add.circle(enemy.x, enemy.y, 5, 0x00ff88);
      this.physics.add.existing(healOrb); healOrb.setData('type', 'heal');
      this.powerups.add(healOrb);
      const dropType = enemy.getData('drop');
      if (dropType) {
        let powerup;
        if (dropType === 'speed') powerup = this.add.circle(enemy.x, enemy.y, 7, 0xffff44);
        else powerup = this.add.rectangle(enemy.x, enemy.y, 14, 14, 0xffff44);
        this.physics.add.existing(powerup); powerup.setData('type', dropType);
  ...
  onCollectPowerup(player, powerup) {
    ...
    if (type === 'heal') {
      ...
      this.healCollected++;
      if (this.healCollected % 50 === 0) { this.bombCount++; this.showBuffText('BOMB +1!'); }
    } else if (type === 'speed') {
  ...
  fireTriangleBomb(angle, bombType) {
    ...
    bomb.setData('detonated', false); bomb.setData('bombType', bombType); bomb.setData('planted', false);
    this.bombs.add(bomb);
  ...
  onBombHitEnemy(bomb, enemy) {
    if (!bomb.active || bomb.getData('detonated')) return;
    if (bomb.getData('bombType') === 'manual') {
      if (!bomb.getData('planted')) this.plantBomb(bomb);
    } else {
  ...
  detonateBomb(bomb) {
    ...
    const gridX = Math.floor(bomb.x / 64), gridY = Math.floor(bomb.y / 64);
    const isManual = bomb.getData('bombType') === 'manual';
    const cells = [];
    if (isManual) {
      for (let dx = -1; dx <= 1; dx++) for (let dy = -1; dy <= 1; dy++) cells.push({ gx: gridX + dx, gy: gridY + dy });
    } else {
      cells.push({ gx: gridX, gy: gridY }, { gx: gridX + 1, gy: gridY },
        { gx: gridX, gy: gridY + 1 }, { gx: gridX + 1, gy: gridY + 1 });
    }
    SFX.bombExplode();
    const fx = this.add.graphics(); fx.setDepth(50);
    cells.forEach(c => {
      fx.fillStyle(0xff6600, 0.35); fx.fillRect(c.gx * 64, c.gy * 64, 64, 64);
      fx.lineStyle(2, 0xff8800, 0.8); fx.strokeRect(c.gx * 64, c.gy * 64, 64, 64);
    });
    this.tweens.add({ targets: fx, alpha: 0, duration: 500, onComplete: () => fx.destroy() });
    bomb.destroy();
    this.time.addEvent({ delay: 50, repeat: 9, callback: () => this.killEnemiesInCells(cells) });
  }
  ...
  killEnemiesInCells(cells) {
    ...
    toKill.forEach(e => {
      ...
      this.physics.add.existing(healOrb); healOrb.setData('type', 'heal'); this.powerups.add(healOrb);
      const dropType = e.getData('drop');
      if (dropType) {
        let powerup;
        if (dropType === 'speed') powerup = this.add.circle(e.x, e.y, 7, 0xffff44);
        else powerup = this.add.rectangle(e.x, e.y, 14, 14, 0xffff44);
        this.physics.add.existing(powerup); powerup.setData('type', dropType); this.powerups.add(powerup);
        this.tweens.add({ targets: powerup, alpha: 0.4, yoyo: true, repeat: -1, duration: 300 });
        this.time.delayedCall(15000, () => { if (powerup.active) powerup.destroy(); });
      }
      e.destroy();
  ...
  spawnDeathParticles(x, y, color) {
    const emitter = this.add.particles(x, y, 'particle', {
      speed: { min: 60, max: 180 }, scale: { start: 0.7, end: 0 },
      lifespan: 350, quantity: 10, tint: color, emitting: false,
    });
    emitter.explode(10);
...
class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(cx, cy - 20, [
      `생존 시간: ${mm}:${ss}`, `웨이브: ${this.stats.wave}`, `킬: ${this.stats.kills}`,
    ].join('\n'), {
      fontSize: '22px', fill: '#ffffff', fontFamily: 'monospace', align: 'center',
    }).setOrigin(0.5);

    const restartBg = this.add.rectangle(cx, cy + 120, 240, 50, 0x446644).setInteractive({ useHandCursor: true });
    this.add.text(cx, cy + 120, '다시 시작', { fontSize: '24px', fill: '#ffffff', fontFamily: 'monospace' }).setOrigin(0.5);
    restartBg.on('pointerdown', () => {
      this.scene.stop('GameOverScene'); this.scene.stop('GameScene');
      this.scene.start('GameScene', { difficulty: this.stats.difficulty });
    });

    const menuBg = this.add.rectangle(cx, cy + 180, 240, 50, 0x444466).setInteractive({ useHandCursor: true });
    this.add.text(cx, cy + 180, '메뉴로', { fontSize: '24px', fill: '#ffffff', fontFamily: 'monospace' }).setOrigin(0.5);
